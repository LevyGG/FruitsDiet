...
class HomePageScroll_VC: UIViewController {

    @IBOutlet weak var collectionView: UICollectionView!
    let identifier = "CellIdentifier"
    var dataSource = DataSource("GoogleStyleImage")
    ...
    func setupFloatWithSize(size:CGSize){
        let flotlayout = HomePageFlowLayout()
        flotlayout.scrollDirection = .horizontal
        flotlayout.minimumLineSpacing = 20.0
        flotlayout.itemSize = CGSize(width: size.width - 100, height: size.height - 60)
        collectionView.collectionViewLayout = flotlayout
...
extension HomePageScroll_VC:UICollectionViewDelegate,UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as!HomeFruitPageScrollCell
...
//        cell.imageView.image = UIImage(named:"Hummer")
